...
from Utils.json_utils import atomic_write_json, load_json
...
def find_latest_schema_result_file(
    ...
    for path in model_root.rglob(f"*_{dataset_name}_*.json"):
...
def resolve_output_path(
    ...
    save_dir = (resolve_project_path(output_dir) if output_dir else None) or (
        logs_dir
        / "sql_results"
...
def main() -> None:
    ...
    prompt_path = resolve_project_path(args.prompt_path) if args.prompt_path else DEFAULT_SQL_PROMPT_PATH
    credential_path = resolve_project_path(args.credential_path) if args.credential_path else None
    ...
    run_info = {
        ...
        "provider": provider,
        "credential_path": None if credential_path is None else str(credential_path),
